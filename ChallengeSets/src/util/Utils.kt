...
val text_characters = listOf(',', '.', ':', ';', '-', '?', '!', '/', '"', '\'') + ('0'..'9')

fun String.etaoinshrdlu() = sumByDouble { c ->
    character_frequencies.getOrDefault(
        c.toLowerCase(),
        if (text_characters.contains(c)) 0.toDouble() else -0.2
    )
}
