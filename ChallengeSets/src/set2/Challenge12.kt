 ...
 * Here's roughly how:
 *
 * Feed identical bytes of your-string to the function 1 at a time --- start with 1 byte ("A"), then "AA", then "AAA"
...
private fun decrypt(oracle: (String) -> ByteArray): String {
    ...
    while (lastResult.isEmpty() || blockSize <= 1) {
        ...
        newResult = oracle("A".repeat(++charsToAdd))
    ...
    (0 until textSizeWithPadding).forEach { index ->
        if (index == cracked.length) {
            val known = "A".repeat(blockSize - ((cracked.length + 1) % blockSize))
            (0..255).asSequence().map { it.toChar().toString() }.firstOrNull {
                oracle(known + cracked + it).copyOfRange(0, known.length + index + 1)
                    .contentEquals(oracle(known).copyOfRange(0, known.length + index + 1))
            }?.apply { cracked += this }
        }
    }
    return cracked
